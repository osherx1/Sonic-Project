...
public static class Utils {
    ...
    // Start hack , "SceneManager.sceneLoaded -=" DOESN'T WORK.
...
{
    // Use refreshRateRatio to calculate the refresh rate
    ...
    // Validate refresh rate and set target frame rate
    Application.targetFrameRate = refreshRate > 0 ? Mathf.RoundToInt(refreshRate) : 60;

    // Set fixedDeltaTime to a stable value
    Time.fixedDeltaTime = 1F / 50F; // 50 physics updates per second

    // Set maximumDeltaTime to allow recovery from frame drops
    Time.maximumDeltaTime = 0.25F; // 250ms max frame time
}
    ...
    // Store all case variants since str.ToLower()
